...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 mb-6">
    <form method="GET" class="flex flex-wrap gap-4">
        ...
        <select name="rol" class="rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <option value="support_admin" {{ request('rol') == 'support_admin' ? 'selected' : '' }}>Support Admin</option>
            <option value="support_agent" {{ request('rol') == 'support_agent' ? 'selected' : '' }}>Support Agent</option>
            <option value="admin" {{ request('rol') == 'admin' ? 'selected' : '' }}>Admin</option>
            <option value="autorizador" {{ request('rol') == 'autorizador' ? 'selected' : '' }}>Autorizador</option>
            <option value="cajero" {{ request('rol') == 'cajero' ? 'selected' : '' }}>Cajero</option>
...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
    <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
        ...
        <a href="{{ route('admin.users.create') }}" class="bg-indigo-600 text-white px-3 py-1.5 rounded-lg text-sm hover:bg-indigo-700">+ Nuevo Usuario</a>
    </div>
    ...
    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
        ...
        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{{ $u->last_login_at ? $u->last_login_at->format('d/m/Y H:i') : 'Nunca' }}</td>
                <td class="px-4 py-3 text-sm">
                    <a href="{{ route('admin.users.edit', $u) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300">Editar</a>
                </td>
            </tr>
            @endforeach
            @else
